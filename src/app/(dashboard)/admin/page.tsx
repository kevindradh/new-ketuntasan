...
export default async function AdminDashboard() {
    ...
    const stats = [
        { label: 'Total Ujian', value: totalExams || 0, icon: FileText, color: 'text-emerald-700', bg: 'bg-emerald-100' },
        { label: 'Lembar Ketuntasan', value: totalSheets || 0, icon: Users, color: 'text-emerald-700', bg: 'bg-emerald-100' },
        { label: 'Sudah Disetujui', value: approvedSheets || 0, icon: CheckCircle2, color: 'text-green-600', bg: 'bg-green-100' },
        { label: 'Menunggu Proses', value: pendingSheets || 0, icon: Clock, color: 'text-emerald-700', bg: 'bg-emerald-100' },
    ]
                ...
                <Card className="border-0 shadow-md">
                    ...
                    <CardContent className="grid grid-cols-2 gap-3">
                        <Link href="/admin/subjects">
                            <div className="p-4 rounded-xl border border-slate-200 hover:border-emerald-200 hover:bg-emerald-50/50 transition-all cursor-pointer group">
                                <BookOpen className="h-8 w-8 text-emerald-700 mb-2 group-hover:scale-110 transition-transform" />
                        ...
                        <Link href="/admin/teachers">
                            <div className="p-4 rounded-xl border border-slate-200 hover:border-emerald-200 hover:bg-emerald-50/50 transition-all cursor-pointer group">
                                <Users className="h-8 w-8 text-emerald-700 mb-2 group-hover:scale-110 transition-transform" />
                        ...
                        <Link href="/admin/exams">
                            <div className="p-4 rounded-xl border border-slate-200 hover:border-emerald-200 hover:bg-emerald-50/50 transition-all cursor-pointer group">
                                <FileText className="h-8 w-8 text-emerald-700 mb-2 group-hover:scale-110 transition-transform" />
                                ...
                                {recentSheets.map((sheet) => (
                                    <div key={sheet.id} className="flex items-center justify-between p-3 rounded-lg bg-slate-50 hover:bg-slate-100 transition-colors">
                                        <div className="flex items-center gap-3">
                                            <div className="h-10 w-10 rounded-full bg-emerald-700 flex items-center justify-center text-white font-medium text-sm">
